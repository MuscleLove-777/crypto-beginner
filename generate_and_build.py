#!/usr/bin/env python3
"""クリプト入門ナビ - GitHub Actions用一括実行スクリプト

キーワード選定 → 記事生成 → アフィリエイト挿入 → SEOチェック → サイトビルド
を一括で実行する。仮想通貨・DeFi・NFTを初心者目線で解説するブログ。
...
def _try_import(module_name, class_name):
    ...
    GitHub Actions環境とローカル開発環境の両方に対応するための
    フォールバック付きインポート関数。
    """
...
def run(cfg=None, prm=None):
    ...
    try:
        ...
        # SEOスコアチェック（最低スコアを下回る場合は警告）
        optimizer = SEOOptimizer(cfg)
        seo_result = optimizer.check_seo_score(article)
        seo_score = seo_result.get("total_score", 0)
        seo_grade = seo_result.get("grade", "?")
        logger.info("SEOスコア: %d/100 (グレード: %s)", seo_score, seo_grade)

        if seo_score < cfg.MIN_SEO_SCORE:
            logger.warning(
                "SEOスコアが基準値(%d)を下回っています: %d",
                cfg.MIN_SEO_SCORE,
                seo_score,
            )
